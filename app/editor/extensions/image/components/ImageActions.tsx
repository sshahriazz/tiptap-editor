...
import {ClipboardCopyIcon, DotSquareIcon, DownloadIcon, Link2Icon, TrashIcon} from "lucide-react";
...
const ActionItems: Array<{
    ...
    {key: 'onView', icon: <TrashIcon className="size-4"/>, tooltip: 'View image'},
        ...
        return (
            <ActionWrapper className={cn({'opacity-100': isOpen})}>
                {shouldMerge ? (
                    <Dropdown isOpen={isOpen} onOpenChange={setIsOpen}>
                        ...
                        <DropdownMenu className="w-56">
                            {filteredActions.map(({key, icon, tooltip}) => (
                                <DropdownItem key={key} onClick={e => handleAction(e, actions[key])}>
                    ...
                    filteredActions.map(({key, icon, tooltip}) => (
                        <ActionButton key={key} icon={icon} tooltip={tooltip}
